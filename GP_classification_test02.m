close all; clear;
% Load Data
load('D:\ops\GPclassification\CollectDatabase\ThrustData\1N-50s\train_input.mat');
load('D:\ops\GPclassification\CollectDatabase\ThrustData\1N-50s\train_output.mat');
load('D:\ops\GPclassification\CollectDatabase\ThrustData\1N-50s\test_data_in_1.mat');
load('D:\ops\GPclassification\CollectDatabase\ThrustData\1N-50s\test_data_out_1.mat');
load('D:\ops\GPclassification\CollectDatabase\ThrustData\1N-50s\test_data_in_2.mat');
load('D:\ops\GPclassification\CollectDatabase\ThrustData\1N-50s\test_data_out_2.mat');
...
%%
  [a b c d lp] = gp(hyp, infunc, meanfunc, covfunc, likfunc, train_input, train_output, test_data_in_2, test_data_out_2);
[test_prediction,test_means,test_variances,test_nlZ] =  gp(hyp, infunc, meanfunc, covfunc, likfunc, train_input, train_output, test_data_in_2);
[train_prediction,train_means,train_variances,train_nlZ] =  gp(hyp, infunc, meanfunc, covfunc, likfunc, train_input, train_output, test_data_in_1);
...
% [p,mu,s2,nlZ] =  binaryGP(hyp, @infEP, covfunc, likfunc, train_input, train_output, test_data_in_2);
%   [me, sig2] =  gp(hyp, @infVB, meanfunc, covfunc, likfunc, train_input, train_output, test_data_in_2);
%% predicted boundary
 [me, sig2] =  gp(hyp, @infVB, meanfunc, covfunc, likfunc, train_input, train_output, test_data_in_2);
...
%%
 ...
 higher_bound = 0.9;
 % test 2
 index_1 = find(test_data_out_2 == 1);
index_0 = find(test_data_out_2 == -1);
output_data_1 = test_data_out_2(index_1);
test_pred_1 = test_prediction(index_1);
output_data_0 = test_data_out_2(index_0);
...
test_size = size(test_data_out_2); test_size = test_size(1);
accuracy_test_2 = num_test/test_size

% test 1
index_1 = []; index_0 = [];
 index_1 = find(test_data_out_1 == 1);
index_0 = find(test_data_out_1 == -1);
train_output_data_1 = test_data_out_1(index_1);
train_pred_1 = train_prediction(index_1);
train_output_data_0 = test_data_out_1(index_0);
...
train_size = size(test_data_out_1); train_size = train_size(1);
accuracy_test_1 = num_train/train_size
...
hold on
plot(test_data_out_2(1:20,1), '--*', 'MarkerSize', 12,'LineWidth',2)
...
subplot(3,1,3)
error = test_data_out_2 - test_prediction;
...
hold on
plot(test_data_out_2, '-*', 'MarkerSize', 12,'LineWidth',2);
...
error_1 = find(test_data_out_2 > test_higher_boundary);
error_2 = find(test_data_out_2 < test_lower_boundary);
...
n = size(LP); n = n(1);
for i = 1:n
    lp_verify(i) = abs(LP(i) * test_data_out_2(i) + (1 - LP(i)) * (-test_data_out_2(i)));
end
lp_verify = lp_verify';
