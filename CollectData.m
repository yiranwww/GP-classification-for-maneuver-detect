...
%         (Magnitude is included)
%         Test-3 the test data is from different orbits from training data
%         (Magnitude is not included)
% Now we have maneuver magnitude from 1 to 10. Set the largest is 10m/s
close all; clear; 
rng(3) % used 3 before 10.3.2020
data_path_name = 'DataCollectionFile\';
save_data_path = 'DataCollectionFile\GPtest\';
%% step1: collect different magnitude maneuvers
km = 1000;
length = 15; % useful track length
gap = 0; % track gap for every two points
measure_gap = 1; % select the data every n point
maneuver = [3 5 10]; % define the training database with maneuver magnitude
part_size = 75; % the percentage of the whole training database
...
for maneu = maneuver
    file_path = [data_path_name, 'maneuver_database\results-20200624-', num2str(maneu), 'mps.mat'];
    load(file_path);
    time_point = measures(:,2);
    database = measures(:, [18:20]); % position with noise
    training_range = 5; 
    t1 = 1; 
    ...
    eval(['total_train_z' '=' '[total_train_z;', 'train_z];']);   % collect the training data 
end
    % basic parameter
    a = 0; % lower
    b = 1; % higher
...
for maneu = maneuver
    ...
    training_range = 5;
    t1 = 1; 
    t2 = 28 * training_range; % training orbit
    t3 = 28* (training_range +1);
    t4 = 28 * 10; % test orbit (with same maneuver as training)
    ...
    for i = 1:training_range
       ...
       each_train_size = round(orbit_size/part_size); % part of the training database
    ...
    test_nor_data_1 = []; test_nor_data_2 = [];
    normal_index = [1,2,3,4,5];
    fix_index = [2,3,4,5,1]; % from different orbits
    for i = 1:training_range
        normal_order = normal_index(i);
        fix_order = fix_index(i);
...
save('D:\ops\GPclassification\Final_code\DataCollectionFile\GP_test\training_input_data.mat', 'training_input_data');
save('D:\ops\GPclassification\Final_code\DataCollectionFile\GP_test\training_output_data.mat', 'training_output_data');
save('D:\ops\GPclassification\Final_code\DataCollectionFile\GP_test\testing_1_input_data.mat', 'testing_1_input_data');
save('D:\ops\GPclassification\Final_code\DataCollectionFile\GP_test\testing_1_output_data.mat', 'testing_1_output_data');
save('D:\ops\GPclassification\Final_code\DataCollectionFile\GP_test\testing_2_input_data.mat', 'testing_2_input_data');
save('D:\ops\GPclassification\Final_code\DataCollectionFile\GP_test\testing_2_output_data.mat', 'testing_2_output_data');

save('D:\ops\GPclassification\Final_code\DataCollectionFile\GP_test\nor_data_parameter.mat', 'Xmin', 'Xmax', 'Ymin', 'Ymax', 'Zmin', 'Zmax', 'kx', 'ky', 'kz', 'a', 'b');


%% Collect the data with unknown maneuver
unknown_maneuver = [1 2 4 6];
...
save('D:\ops\GPclassification\Final_code\DataCollectionFile\GP_test\testing_3_input_data.mat', 'testing_3_input_data');
save('D:\ops\GPclassification\Final_code\DataCollectionFile\GP_test\testing_3_output_data.mat', 'testing_3_output_data');

save('D:\ops\GPclassification\Final_code\DataCollectionFile\GP_test\AllData.mat');
